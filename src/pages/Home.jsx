    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
        ...
        {mobileMenu && (
          <div className="border-t border-slate-100 bg-white px-5 py-4 md:hidden">
            <div className="flex flex-col gap-4">
              <button className="text-left font-medium">Home</button>
              <button className="text-left font-medium">Search</button>
              <button className="text-left font-medium">Store Map</button>
              <button className="text-left font-medium">My List</button>
            </div>
          </div>
        )}
      </header>
        ...
        <section className="mx-auto max-w-7xl px-5 pb-8 pt-10 sm:px-6 lg:px-8 lg:pb-14 lg:pt-16">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            ...
            <div>
              ...
              {/* SEARCH */}
              <div className="mt-8 flex max-w-2xl items-center rounded-2xl border border-blue-200 bg-white px-5 shadow-[0_8px_30px_rgba(37,99,235,0.10)] transition duration-300 focus-within:border-blue-500 focus-within:shadow-[0_10px_35px_rgba(37,99,235,0.18)] hover:shadow-lg">
                <Search
                  size={28}
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {popular.map((item) => (
                    <button
                      key={item.name}
                      onClick={() => selectPopular(item.name)}
                      className={`flex items-center gap-2 rounded-full border px-4 py-2.5 transition duration-200 active:scale-95 ${
                        selectedProduct.category === item.name
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 grid grid-cols-4 border-t border-slate-200 bg-white px-3 pb-3 pt-3 shadow-[0_-5px_20px_rgba(15,23,42,0.05)] md:hidden">
        {[
          ...
          <button
            key={item.label}
            className={`flex flex-col items-center gap-1 rounded-xl py-1 transition duration-200 active:scale-90 ${
              index === 0
                ? "text-blue-600"
